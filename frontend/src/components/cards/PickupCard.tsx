...
export default function PickupCard({ p }: { p: PickupVM }) {
  ...
  const okLabel = { ko: "가능", ja: "可能", en: "Available" }[lang];
    ...
    <div className="flex flex-col gap-[11px] rounded-lg border border-line bg-card px-[15px] py-3.5 shadow-card">
      ...
      <div className="flex gap-2">
        <div className="flex-1 rounded-xs bg-canvas px-[11px] py-[9px]">
          <div className="mb-0.5 text-[10.5px] text-gray">{T.pickupSlot}</div>
          {/* 수치·시각은 언어 무관 원본 그대로 (§7-③) */}
          <div className="text-[13.5px] font-bold text-ink">{p.slot}</div>
        </div>
        <div className="flex-1 rounded-xs bg-canvas px-[11px] py-[9px]">
          <div className="mb-0.5 text-[10.5px] text-gray">{T.pickupDeadline}</div>
          <div className="text-[13.5px] font-bold text-ink">{p.deadline}</div>
        </div>
      </div>
      <button
        onClick={p.onReserve}
